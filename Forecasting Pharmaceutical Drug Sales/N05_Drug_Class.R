...
# PHARMACEUTICAL DRUG SALES VOLUME DATA | N05 
...
df %>% 
    ...
    ggtitle('Pharmaceutical Drug Class: N05', subtitle = 'Weekly Sales Volume') + 
    labs(x = '', y = 'Volume') + 
...
autoplot(x, ts.alpha = 0.5) + 
    ...
    ggtitle('Pharmaceutical Drug Class: N05') + 
...
fc %>% accuracy(test$N05) 
sw_sweep(fc) %>% 
    ggplot(aes(x = index, y = N05, color = key)) + 
    geom_ribbon(aes(ymin = lo.95, ymax = hi.95), fill = 'skyblue') + 
    geom_ribbon(aes(ymin = lo.80, ymax = hi.80), fill = 'slateblue', alpha = 0.7) + 
    geom_line(size = 1) + 
    labs(title = 'N05 Validation Forecast', x = '', y = 'Volume', 
         subtitle = 'ARIMA(3, 1, 2)') + 
    scale_x_yearmon(n = 6, format = '%Y') + 
    theme_tq()
...
sw_sweep(fc_n05) %>% 
    ...
    geom_ribbon(aes(ymin = lo.80, ymax = hi.80), fill = 'slateblue', alpha = 0.7) + 
    geom_line(size = 1) + 
    labs(title = 'N05 Forecast: Last 11 Weeks of 2019', x = '', y = 'Volume', 
         subtitle = 'ARIMA(3, 1, 2)') + 
    scale_x_yearmon(n = 6, format = '%Y') + 
    theme_tq()
